locals {
  ...
  lt_security_group_ids = [
    for key, value in module.security_groups.sg : value if contains(var.launch_template_sg, key)
  ]
  ...
  db_subnet_ids = [
    for key, value in module.vpc.db_subnet_ids : value
  ]

  db_security_group_ids = [
    for key, value in module.security_groups.sg : value if key == "rds"
  ]
